...
build_syslinux() {
	...
	fallocate -l 37 "$DESTDIR"/UUID # UUID in compliant RFC 4122 v4 format
	fallocate -l 13 "$DESTDIR"/HOSTNAME # HOSTANME with 12 ASCII characters in compliant RFC 1123 format
	rm -rf "$DESTDIR"/usr
}
