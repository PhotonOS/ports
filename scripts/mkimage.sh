...
--name			Specify prefix name of archive (ie. alpine)
...
--version		Specify version to use in Output filename
...
build_profile() {
	...
	# Defaults
	[ -n "$image_name" ] || image_name="${NAME}"
	[ -n "$output_filename" ] || output_filename="${image_name}-${RELEASE}-${VERSION}-${ARCH}.${image_ext}"
